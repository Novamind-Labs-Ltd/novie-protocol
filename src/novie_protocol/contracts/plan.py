...
class ExecutionStep:
    ...
    # into PMS for later execution re-planning. ``routing_target`` is the
    # preferred runtime/handoff target. Both are optional during the migration
    # window; downstream consumers must treat ``None`` as "legacy / unspecified"
    # and fall back to current behaviour.
    execution_mode: StepExecutionMode | None = None
    routing_target: StepRoutingTarget | None = None
